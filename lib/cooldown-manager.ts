...
export function recordRefresh(talentUuid: string): void {
  ...
  console.log(`[CooldownManager] Recorded refresh for ${talentUuid}, expires at ${new Date(entry.expiresAt).toLocaleTimeString()}`);
}
...
export function cleanupExpiredCooldowns(): number {
  ...
  if (removedCount > 0) {
    console.log(`[CooldownManager] Cleaned up ${removedCount} expired cooldown entries`);
  }
...
export function useCooldownTracker(talentUuid: string) {
  const [cooldownMinutes, setCooldownMinutes] = React.useState<number | null>(null);
