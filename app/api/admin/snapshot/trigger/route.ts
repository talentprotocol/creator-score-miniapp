...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create snapshot (this will delete existing snapshot first)
    const result =
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Delete snapshot using the service method
    ...
    if (error) {
      console.error("[Admin Snapshot Trigger] Error deleting snapshot:", error);
      return NextResponse.json(
