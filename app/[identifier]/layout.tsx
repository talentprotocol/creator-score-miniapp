...
export default async function ProfileLayout({
  ...
  credentials.forEach((credentialGroup) => {
    credentialGroup.points.forEach((point) => {
      ...
      // Store raw data by slug for business logic
      rawCollectorData.set(point.slug || "", value);
    });
  });

  // Business logic: adjust for double-counting using slugs
  const mirrorCollectors = rawCollectorData.get("mirror_unique_collectors") || 0;
  const openseaCollectors = rawCollectorData.get("opensea_nft_total_owners") || 0;
  
  // Calculate adjusted OpenSea count (subtract Mirror to avoid double-counting)
  const adjustedOpenseaCollectors = Math.max(0, openseaCollectors - mirrorCollectors);
